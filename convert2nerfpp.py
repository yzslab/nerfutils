...
if __name__ == "__main__":
    def parser_configure(parser):
        parser.add_argument("--scene-scale", default=1,
                            type=float, help="scale the scene that put all camera into unit sphere")
    ...
    if os.path.exists(image_directory) is False:
        image_directory = os.path.join(os.path.dirname(args.transforms_npy), image_directory)
    ...
    for image_filename in transform_matrix:
        ...
        intrinsics_matrix = transforms_and_cameras["intrinsics_matrix"][transforms_and_cameras["image_camera_id"][image_filename]]
